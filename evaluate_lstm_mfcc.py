...
for j in range(5):
    evaluation = Evaluation('mnt/0/lstm_mel_distances_5712.npy', 'mnt/0/useful_playlists', 'mnt/0/useful_songs', False, threshold=0.994)
    ...
    for user in evaluation.users:
        ...
        i = i + 1
    filename = 'mnt/0/results/lstm_mel_results_5712/chopped_lstm_mel_' + str(j+1)
    print(results.shape)
    results.to_csv(filename, sep=';', header=False, index=False)


for j in range(5):
    evaluation = Evaluation('mnt/0/lstm_mfcc_distances.npy', 'mnt/0/useful_playlists', 'mnt/0/useful_songs', False, threshold=0.9978605987758714)
    results = pandas.DataFrame(columns=['playlist_lenght', 'test_list_lenght', 'number_of_matches', 'match_ranking', 'recall_at_10',
                 'recall_at_50', 'recall_at_100', 'nDGC'])
    i = 0
    for user in evaluation.users:
        ...
        results = results.append(temp_frame)
        i = i + 1
    filename = 'mnt/0/results/lstm_mfcc_results/chopped_lstm_mfcc_' + str(j+1)
    print(results.shape)
    results.to_csv(filename, sep=';', header=False, index=False)